...
Releves.dataContext = (function ($) {
    ...
    var getReleveById = function (id) {
        var releve = null;
        relevesList.forEach(function (tmpReleve) {
            if (tmpReleve.id === id) {
    ...
    var getSensorsForChart = function () {
        ...
        var timestamp;
        relevesList.forEach(function (releve) {
            if (releve.sensor1 !== "") {
                var sensor1 = [];
                timestamp = releve.id * 1000;
                sensor1[0] = timestamp;
                sensor1[1] = releve.sensor1;
                s1.push(sensor1);
                var sensor2 = [];
                sensor2[0] = timestamp;
                sensor2[1] = releve.sensor2;
                s2.push(sensor2);
                var sensor3 = [];
                sensor3[0] = timestamp;
                sensor3[1] = releve.sensor3;
                s3.push(sensor3);
            }
        });
        sensors.s1 = s1;
